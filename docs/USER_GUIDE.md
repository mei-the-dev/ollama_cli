...
## Configuration (`~/.omarchy/config.json`)
Key fields:
...
1. Transfer knowledge (loads `~/.omarchy/knowledge/*.json` into system prompt)
2. Transfer conversation context (loads `~/.omarchy/context/context.json`)
...
## Development & Tests
Run tests with `./.venv/bin/python -m pytest -q`. Use `OMARCHY_SKIP_OLLAMA=1` in CI.
