# Singularity — AI Code Agent

Singularity is a local AI-powered coding assistant that uses a Model Context Protocol (MCP) server to provide code-aware tools such as `write_code`, `apply_edit`, `fetch_url`, and more.
...
- Safety: `sudo` is opt-in (`~/.singularity/config.json`) and `auto_apply` for file writes is opt-in
...
Dashboard
- A lightweight TUI dashboard is available at `ref/singularity_dashboard.py` (or copy to `~/.singularity/singularity_dashboard.py`). See `docs/DASHBOARD.md` for usage and quick start tips.
