...
def test_parsed_tool_write_code(model_event_logger, event_reader):
    """Parsed tool: model should emit write_code tool"""
    # Emit the prompt and assistant events deterministically
    model_event_logger.emit('PROMPT', {'prompt': "write a file with content hello"})
