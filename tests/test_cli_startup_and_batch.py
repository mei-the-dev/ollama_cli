...
def test_cli_startup_and_batch_file_ops(tmp_path):
    ...
    proc = subprocess.Popen(
        script_cmd + ["--startup-check-only", "--startup-wait", "3"],
    ...
    try:
        ...
        payload = {
            "name": "read_code",
            "arguments": {"filepath": str(tmp_path / "batch_file.txt")},
        }
        req = urllib.request.Request(
            url + "/call",
            data=json.dumps(payload).encode("utf-8"),
        ...
        with urllib.request.urlopen(req, timeout=3) as r:
            ...
            data = resp.get("data")
            assert (
                "hello from batch" in data.get("content", "")
                or Path(tmp_path / "batch_file.txt").read_text() == "hello from batch"
            )
