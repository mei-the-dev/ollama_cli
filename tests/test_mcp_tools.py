...
async def test_write_and_apply_edit(tmp_path):
    ...
    # Preview an edit
    edit_preview = await server.apply_edit(
        {"filepath": str(target), "search": "x+y", "replace": "x + y", "preview": True}
    )
    assert edit_preview.status == ToolStatus.SUCCESS
    assert "preview" in edit_preview.data
    # Apply the edit
    edit_apply = await server.apply_edit(
        {"filepath": str(target), "search": "x+y", "replace": "x + y", "apply": True}
    )
...
async def test_create_project_and_venv(tmp_path, monkeypatch):
    ...
    server = MCPServer()
    res = await server.create_project(
        {
            "name": "proj",
            "template": "python-api",
            "create_venv": True,
...
async def test_refactor_rename(tmp_path):
    ...
    res = await server.refactor_code(
        {
            "filepath": str(f),
    ...
    # apply
    res2 = await server.refactor_code(
        {
            "filepath": str(f),
            "refactor_type": "rename",
            "old": "old_name",
...
def test_cli_tool(tmp_path, monkeypatch):
    ...
    out = subprocess.run(
        [sys.executable, "scripts/mcp_tool.py", "write_code", args],
        capture_output=True,
        text=True,
    )
