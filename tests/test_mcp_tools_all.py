...
def test_write_and_read_code(mcp, tmp_path):
    ...
    out = asyncio.run(
        mcp.write_code(
            {"filepath": str(target), "content": "hello world", "mode": "overwrite"}
        )
    )
...
def test_generate_and_run_tests(mcp, tmp_path):
    ...
    assert (
        proc.returncode == 0
    ), f"pytest failed: stdout={proc.stdout}\nstderr={proc.stderr}"
...
def test_create_and_update_plan(mcp):
    ...
    create = asyncio.run(
        mcp.create_plan({"goal": "Test plan", "auto_break_down": True})
    )
    assert create.status == ToolStatus.SUCCESS
    pid = create.data["plan_id"]
    upd = asyncio.run(
        mcp.update_plan({"plan_id": pid, "item_id": "1", "status": "complete"})
    )
...
def test_save_and_query_knowledge(mcp):
    ...
    sk = asyncio.run(
        mcp.save_knowledge(
            {"topic": "Test", "content": "This is a test", "tags": ["test"]}
        )
    )
...
def test_http_call_endpoint(tmp_path):
    ...
    proc = subprocess.Popen(
        [sys.executable, str(script), "--host", "127.0.0.1", "--port", "0"],
