...
if __name__ == "__main__":
            ...
            if kb_dir.exists():
                ...
                for f in list(kb_dir.glob("*.json"))[:10]:
                    try:
                        ...
                        entries.append(
                            f"{j.get('topic')} - { (j.get('content','')[:200] + '...') if len(j.get('content',''))>200 else j.get('content','')} "
                        )
                    except Exception:
                        continue
                if entries:
    ...
    async def handle_tool_calls(self, tool_calls: List[Dict]):
        ...
        for tool_call in tool_calls:
            ...
            auto_apply = bool(os.environ.get("SINGULARITY_AUTO_APPLY")) or getattr(
            ...
            if tool_name == "write_code" and not auto_apply:
                if interactive:
                    try:
                        confirm = Confirm.ask(
                            f"Apply write to {tool_args.get('filepath')}? (y/n)"
                        )
                    except Exception:
                        confirm = False
                else:
                ...
                if not confirm:
                    console.print(
                        f"[yellow]Skipped writing {tool_args.get('filepath')}[/yellow]"
                    )
                    continue
            ...
            try:
                if not hasattr(self, "mcp_server_url") or not self.mcp_server_url:
                    console.print(
                        f"[red]MCP server URL not known; cannot execute tool {tool_name}[/red]"
                    )
                    continue
                async with aiohttp.ClientSession() as session:
                    payload = {"name": tool_name, "arguments": tool_args}
                    async with session.post(
                        self.mcp_server_url + "/call", json=payload
                    ) as resp:
                        res = await resp.json()
                        console.print(
                            f"  [green]{COMPLETE_SYMBOL}[/green] {tool_name} completed: {res.get('status', res)}"
                        )
    ...
    def display_plan(self, plan: Dict):
        ...
        table = Table(
            title="📋 Current Plan", show_header=True, header_style="bold cyan"
        )
...
class SingularityCLI:
    ...
    def show_banner(self):
        """Display the Omarchy banner with a stylized layout"""
        console.clear()
    ...
    def show_help(self):
        ...
        commands = [
            ...
            (
                "/dashboard",
                "Launch the dashboard (opens a tmux window or background process)",
            ),
            (
                "/config show",
                "Show effective configuration (env + ~/.omarchy/config.json)",
            ),
            ("/help", "Show this help"),
            ("/exit", "Exit Omarchy"),
        ]
    ...
    def get_effective_config(self) -> Dict:
        ...
        conf = {
            "allow_sudo": env_bool(
                "SINGULARITY_ALLOW_SUDO", file_cfg.get("allow_sudo", False)
            ),
            "auto_apply": env_bool(
                "SINGULARITY_AUTO_APPLY", file_cfg.get("auto_apply", False)
            ),
            "skip_ollama": env_bool("SINGULARITY_SKIP_OLLAMA", False),
            "model": os.environ.get(
                "SINGULARITY_MODEL", file_cfg.get("model", self.agent.model)
            ),
            "mcp_server_url": getattr(self.agent, "mcp_server_url", None),
            "preferred_terminal": os.environ.get(
                "SINGULARITY_PREFERRED_TERMINAL", file_cfg.get("preferred_terminal")
            ),
        }
        return conf
        ...
        if tmux_path:
            ...
            if os.environ.get("DISPLAY") or os.environ.get("WAYLAND_DISPLAY"):
                ...
                emulators = [
                    ...
                    (
                        "x-terminal-emulator",
                        lambda e, s: [e, "-e", "tmux", "attach", "-t", s],
                    ),
                ...
                for name, make_args in emulators:
                    ...
                    if em_path:
                        try:
                            ...
                            console.print(
                                f"[green]Opened {name} and attached to tmux session '{session_name}'.[/green]"
                            )
                            break
                        except Exception:
                ...
                console.print(
                    "[dim]No graphical display detected; not opening a terminal emulator automatically.[/dim]"
                )
        else:
            ...
            console.print(
                f"[green]Dashboard started (background pid: {proc.pid}). Logs: {dashboard_log}[/green]"
            )
            # Discard output but also write a small starter log
            try:
                with open(dashboard_log, "ab") as f:
                    f.write(
                        f"Started dashboard pid={proc.pid} at {datetime.now().astimezone().isoformat()}\n".encode(
                            "utf-8"
                        )
                    )
            except Exception:
