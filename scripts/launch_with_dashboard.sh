...
SINGULARITY_SESSION="singularity"
OMARCHY_SESSION="$SINGULARITY_SESSION"
...
start_tmux_session() {
  ...
  if [[ "$SKIP_OLLAMA" != "1" ]]; then
    tmux send-keys -t "$SINGULARITY_SESSION:ollama" "echo 'Starting Ollama serve...' && ollama serve 2>&1 | tee -a \"$LOG_DIR/ollama.log\"" C-m
  else
    tmux send-keys -t "$OMARCHY_SESSION:ollama" "echo 'SKIP_OLLAMA=1 set — not starting ollama'" C-m
  fi

tmux new-window -t "$SINGULARITY_SESSION" -n "mcp"
tmux send-keys -t "$SINGULARITY_SESSION:mcp" "echo 'Starting MCP server...' && python3 $REPO_ROOT/mcp_server.py 2>&1 | tee -a \"$LOG_DIR/mcp_server.log\"" C-m

  # wait until MCP is ready, then attach CLI and dashboard windows
  tmux new-window -t "$SINGULARITY_SESSION" -n "wait" 
  tmux send-keys -t "$SINGULARITY_SESSION:wait" "echo 'Waiting for services to be ready...' && \"$0\" --wait-ready" C-m

  # CLI and dashboard windows will be created by the wait-ready branch
  tmux attach-session -t "$SINGULARITY_SESSION"
}
...
start_background_processes() {
  ...
  echo "Starting CLI (singularity; legacy 'omarchy' alias supported) and dashboard in background logs..."
  (echo "Starting CLI..."; (command -v singularity >/dev/null 2>&1 && exec singularity) || python3 "$REPO_ROOT/singularity_cli.py") >>"$LOG_DIR/cli.log" 2>&1 &
...
if [[ "${1:-}" == "--wait-ready" ]]; then
  ...
  if [[ "$SKIP_OLLAMA" != "1" ]]; then
    if wait_for_http "http://localhost:11434/api/tags" 30; then
      tmux new-window -t "$SINGULARITY_SESSION" -n "cli"
      tmux send-keys -t "$SINGULARITY_SESSION:cli" "echo 'Starting CLI...' && (command -v singularity >/dev/null 2>&1 && exec singularity) || python3 $REPO_ROOT/singularity_cli.py" C-m
    else
      tmux new-window -t "$SINGULARITY_SESSION" -n "cli"
      tmux send-keys -t "$SINGULARITY_SESSION:cli" "echo 'Ollama not ready — you can start the CLI manually'" C-m
    fi
  else
    tmux new-window -t "$OMARCHY_SESSION" -n "cli"
    tmux send-keys -t "$OMARCHY_SESSION:cli" "echo 'SKIP_OLLAMA set — starting CLI' && (command -v singularity >/dev/null 2>&1 && exec singularity) || python3 $REPO_ROOT/singularity_cli.py" C-m
  fi
  ...
  if wait_for_mcp 30; then
    ...
    tmux new-window -t "$SINGULARITY_SESSION" -n "dashboard"
    tmux send-keys -t "$SINGULARITY_SESSION:dashboard" "echo 'MCP not ready — start dashboard manually after MCP is up'" C-m
  fi
...
if command -v tmux >/dev/null 2>&1; then
  echo "tmux found — launching services in tmux session '${SINGULARITY_SESSION:-singularity}' (legacy 'omarchy' supported)"
  start_tmux_session
else
