...
class SingularityMCPServer:
    ...
    def _log_convergence(self, operation: str, duration_ms: float):
        ...
        try:
            with open(self.convergence_log, 'a') as f:
                f.write(f"{datetime.now().isoformat()} | {operation} | {duration_ms:.2f}ms\n")
    ...
    async def http_handler(self, request):
        ...
        try:
            ...
            return web.json_response({"error": "invalid convergence request"}, status=400)
        ...
        if not handler:
            return web.json_response({"error": f"Unknown convergence tool: {tool_name}"}, status=400)
        ...
        try:
            ...
            return web.json_response({
                "status": result.status.value if isinstance(result, ToolResult) else "SUCCESS",
    ...
    async def health(self, request):
        ...
        return web.json_response({
            ...
            "top_patterns": dict(sorted(self.convergence_patterns.items(), key=lambda x: x[1], reverse=True)[:5])
        })
    ...
    async def materialize_file(self, args: Dict) -> ToolResult:
        ...
        try:
            ...
            create_backup = args.get("create_backup", self.config.get("auto_apply", True))
    ...
    async def read_file(self, args: Dict) -> ToolResult:
        ...
        try:
            ...
            if not filepath.exists():
                return ToolResult(
                    status=ToolStatus.ERROR,
                    error="File not in convergence space"
                )
    ...
    async def analyze_convergence_space(self, args: Dict) -> ToolResult:
        ...
        try:
            ...
            if filepath.suffix == ".py":
                analysis["functions"] = len(re.findall(r'^def \w+', content, re.MULTILINE))
                analysis["classes"] = len(re.findall(r'^class \w+', content, re.MULTILINE))
                analysis["imports"] = len(re.findall(r'^import |^from .+ import', content, re.MULTILINE))
            elif filepath.suffix in [".js", ".ts"]:
                analysis["functions"] = len(re.findall(r'function \w+|const \w+ = (?:async )?(?:\([^)]*\)|[^=]+) =>', content))
