...
class SingularityAgent:
    ...
    async def start_mcp_server(self, timeout: float = 5.0):
        ...
        if not self.mcp_server_path.exists():
            ...
            if repo_path.exists():
                ...
                console.print(
                    "[yellow]MCP server not found. Please install it first (or copy mcp_server.py to ~/.singularity/).[/yellow]"
                )
                return None
        ...
        try:
            ...
            if port:
                console.print(
                    f"[green]MCP server running at {self.mcp_server_url}[/green]"
                )
                logger.info("MCP server running at %s", self.mcp_server_url)
            else:
                console.print(
                    "[yellow]MCP server started but did not report port within timeout[/yellow]"
                )
                logger.warning(
                    "MCP server started but did not report port within timeout"
                )
            return process
    ...
    async def execute_with_animation(
        self, prompt: str, task_description: str, system: Optional[str] = None
    ):
